...
public class SecurityConfig {
	 ...
	 * "buzz", encoder.encode("password"), Arrays.asList(new
	 * SimpleGrantedAuthority("ROLE_USER")))); usersList.add(new User( "woody",
	 * encoder.encode("password"), Arrays.asList(new
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		return http.authorizeRequests().antMatchers("/design", "/orders").access("hasRole('USER')")
				...
				.defaultSuccessUrl("/design", true).and().logout().logoutSuccessUrl("/").and().build();
	}
}
